...
counts_pseudoNgir <- read.table("clark_counts_VgirRef.txt", header = TRUE, sep = "\t")
counts_Nvit <- read.table("clark_counts.txt", header = TRUE, sep = "\t")
DF <- data.frame(counts_Nvit, counts_pseudoNgir)
colnames = c("SRR1566022", "SRR1566023", "SRR1566024", 
             "SRR1566028", "SRR1566029", "SRR1566030", 
             "SRR2773794", "SRR2773795", "SRR2773796", 
             "SRR2773797", "SRR2773799")
counts <- sapply(colnames, function(x) rowMeans(DF [, grep(x, names(DF))] )  )
genes <- read.table("genes.txt", header = TRUE, sep = "\t")
pheno <- read.table("clark_pheno.csv", header = TRUE, sep = ",")

removals <- c("SRR2773798")
#removals <- c()
samplesToRemove <- c(removals) # update depending on comparison being made

SAMPLE_LENGTH<-as.numeric(length(samplesToRemove)) # to call later
half_sample_length <- SAMPLE_LENGTH/2 # half the sample length
#removals <- (names(counts_pseudoNgir) %in% samplesToRemove[1:SAMPLE_LENGTH]) # for matching names create a value of TRUE or FALSES
...
colnames(design) <- gsub("dge\\$samples\\$strain", "", colnames(design))
head(design)
...
png(filename ="figures/clark_AvgREFs_mds_1and2.png", width= 450, height= 450)
mds <- plotMDS(v, top = 50, ndim = 10, dim.plot = c(1,2), plot=TRUE, cex=2,
               pch=ifelse(v$targets$strain %in% c("VV"), 17,
                          ifelse(v$targets$strain %in% c("GG"), 15,
                                 ifelse(v$targets$strain %in% c("VG"), 9,  12))),
               col=ifelse(v$targets$strain=="VV",
                          VV_Color,
                          ifelse(v$targets$strain=="GG",
                                 GG_Color,
...
# Adding legends
legend("topleft", pch=c(17, 15, 9, 12), 
       col=c(VV_Color, GG_Color, VG_Color, GV_Color),
       legend=c("VV", "GG", "VG", "GV"))
dev.off()
dev.off()
# MDS plot of the first two dimensions using top50 genes. The dimensions are in the mds object.
png(filename ="figures/clark_AvgREFs_mds_2and3.png", width= 450, height= 450)
...
# Plotting
png(filename ="figures/clark_AvgREFs_PCA.png", width= 650, height= 650)
ggplot(data=pcaWithMetadata, aes(x = PC1, y = PC2, shape = strain,
                                 color = strain)) +
  geom_point(size = 8) +
  theme_bw() +
  xlim(-150,150)+
...
colnames(design) <- gsub("v\\$targets\\$strain", "", colnames(design))
head(design)
...
contrasts <- makeContrasts(VV_vs_GG = VV - GG,
                           VV_vs_VG = VV - VG,
                           VV_vs_GV = VV - GV,
                           GG_vs_VG = GG - VG,
                           GG_vs_GV = GG - GV,
...
sumTable <- summary(decideTests(vfit, adjust.method = "BH", p.value = 0.01, lfc = 2))
...
for (i in allComparisons){
  ...
  path <- paste("./DEGs/DEGs_clark_AvgREFs_fpkm05_", i, "_fdr1_lfc0.txt", sep = "")
  write.table(vTopTableAll, path, sep = "\t")
  
  # Adj.p<0.05, log2FC>1
  vTopTable1 <- topTable(veBayesFit, coef=coef, n=Inf, p.value=0.05, lfc=1)
  path <- paste("./DEGs/DEGs_clark_AvgREFs_fpkm05_", i, "_fdr05_lfc1.txt", sep = "")
  write.table(vTopTable1, path, sep = "\t")
  
  # Adj.p<0.01, log2FC>0
  vTopTable1 <- topTable(veBayesFit, coef=coef, n=Inf, p.value=0.01, lfc=0)
  path <- paste("./DEGs/DEGs_clark_AvgREFs_fpkm05_", i, "_fdr001_lfc0.txt", sep = "")
  write.table(vTopTable1, path, sep = "\t")
  
  # Adj.p<0.01, log2FC>1
  vTopTable2 <- topTable(veBayesFit, coef=coef, n=Inf, p.value=0.01, lfc=1)
  path <- paste("./DEGs/DEGs_clark_AvgREFs_fpkm05_", i, "_fdr001_lfc1.txt", sep = "")
  write.table(vTopTable2, path, sep = "\t")
  
  # Adj.p<0.01, log2FC>2
  vTopTable3 <- topTable(veBayesFit, coef=coef, n=Inf, p.value=0.01, lfc=2)
  path <- paste("./DEGs/DEGs_clark_AvgREFs_fpkm05_", i, "_fdr001_lfc2.txt", sep = "")
...
VV_GG_DEG <- rownames(read.table("./DEGs/DEGs_clark_AvgREFs_fpkm05_VV_vs_GG_fdr001_lfc2.txt"))
VV_VG_DEG <- rownames(read.table("./DEGs/DEGs_clark_AvgREFs_fpkm05_VV_vs_VG_fdr001_lfc2.txt"))
VV_GV_DEG <- rownames(read.table("./DEGs/DEGs_clark_AvgREFs_fpkm05_VV_vs_GV_fdr001_lfc2.txt"))

venn.plot <- VennDiagram:::venn.diagram(
  x = list("VV_GG_DEG" = VV_GG_DEG, "VV_VG_DEG" = VV_VG_DEG, "VV_GV_DEG" = VV_GV_DEG),
  filename = "clark_AvgREFs_VV_comparisons_venn.tiff",
  scaled = TRUE,
  col = "transparent",
...
# GG comparison venn
VV_GG_DEG <- rownames(read.table("./DEGs/DEGs_clark_AvgREFs_fpkm05_VV_vs_GG_fdr001_lfc2.txt"))
GG_VG_DEG <- rownames(read.table("./DEGs/DEGs_clark_AvgREFs_fpkm05_GG_vs_VG_fdr001_lfc2.txt"))
GG_GV_DEG <- rownames(read.table("./DEGs/DEGs_clark_AvgREFs_fpkm05_GG_vs_GV_fdr001_lfc2.txt"))

venn.plot <- VennDiagram:::venn.diagram(
  x = list("VV_GG_DEG" = VV_GG_DEG, "GG_VG_DEG" = GG_VG_DEG, "GG_GV_DEG" = GG_GV_DEG),
  filename = "clark_AvgREFs_GG_comparisons_venn.tiff",
  scaled = TRUE,
  col = "transparent",
...
# Volcano plots. 
# VV vs GG
VV_vs_GG_DEG <- read.table("./DEGs/DEGs_clark_AvgREFs_fpkm05_VV_vs_GG_fdr1_lfc0.txt", header=TRUE, sep="\t", stringsAsFactors=F) 
VV_vs_GG_DF <- data.frame(VV_vs_GG_DEG$adj.P.Val, VV_vs_GG_DEG$logFC, VV_vs_GG_DEG$Chr, VV_vs_GG_DEG$Geneid)
colnames(VV_vs_GG_DF) <- c("adj.P.Val", "logFC", "Chr", "Geneid")

VV_vs_GG_DF_Sig <- VV_vs_GG_DF[(abs(VV_vs_GG_DF$logFC) >= 2 & VV_vs_GG_DF$adj.P.Val <= 0.01),]$Geneid
...
up_VV <- subset(VV_vs_GG_DF, VV_vs_GG_DF$logFC >= 2 & VV_vs_GG_DF$adj.P.Val <= 0.01  & (Geneid %in% VV_vs_GG_DF_Sig))
up_VV <- cbind(up_VV , rep(2, nrow(up_VV)))
colnames(up_VV)[5] <- "Color"

up_GG <- subset(VV_vs_GG_DF, VV_vs_GG_DF$logFC <= -2 & VV_vs_GG_DF$adj.P.Val <= 0.01  & (Geneid %in% VV_vs_GG_DF_Sig))
...
p
png(filename ="figures/clark_AvgREFs_VV_vs_GG_valcano.png", width= 650, height= 650)
...
# VV vs VG
VV_vs_VG_DEG <- read.table("./DEGs/DEGs_clark_AvgREFs_fpkm05_VV_vs_VG_fdr1_lfc0.txt", header=TRUE, sep="\t", stringsAsFactors=F) 
VV_vs_VG_DF <- data.frame(VV_vs_VG_DEG$adj.P.Val, VV_vs_VG_DEG$logFC, VV_vs_VG_DEG$Chr, VV_vs_VG_DEG$Geneid)
colnames(VV_vs_VG_DF) <- c("adj.P.Val", "logFC", "Chr", "Geneid")

VV_vs_VG_DF_Sig <- VV_vs_VG_DF[(abs(VV_vs_VG_DF$logFC) >= 2 & VV_vs_VG_DF$adj.P.Val <= 0.01),]$Geneid
...
up_VV <- subset(VV_vs_VG_DF, VV_vs_VG_DF$logFC >= 2 & VV_vs_VG_DF$adj.P.Val <= 0.01  & (Geneid %in% VV_vs_VG_DF_Sig))
up_VV <- cbind(up_VV , rep(2, nrow(up_VV)))
colnames(up_VV)[5] <- "Color"

up_VG <- subset(VV_vs_VG_DF, VV_vs_VG_DF$logFC <= -2 & VV_vs_VG_DF$adj.P.Val <= 0.01  & (Geneid %in% VV_vs_VG_DF_Sig))
...
# Adding lines for significance thresholds
p
png(filename ="figures/clark_AvgREFs_VV_vs_VG_valcano.png", width= 650, height= 650)
p + geom_hline(yintercept = 2, colour="#000000", linetype="dashed") +
  geom_vline(xintercept = 2, colour="#000000", linetype="dashed") + 
  geom_vline(xintercept = -2, colour="#000000", linetype="dashed")
dev.off()
...
# VV vs GV
VV_vs_GV_DEG <- read.table("./DEGs/DEGs_clark_AvgREFs_fpkm05_VV_vs_GV_fdr1_lfc0.txt", header=TRUE, sep="\t", stringsAsFactors=F) 
VV_vs_GV_DF <- data.frame(VV_vs_GV_DEG$adj.P.Val, VV_vs_GV_DEG$logFC, VV_vs_GV_DEG$Chr, VV_vs_GV_DEG$Geneid)
colnames(VV_vs_GV_DF) <- c("adj.P.Val", "logFC", "Chr", "Geneid")

VV_vs_GV_DF_Sig <- VV_vs_GV_DF[(abs(VV_vs_GV_DF$logFC) >= 2 & VV_vs_GV_DF$adj.P.Val <= 0.01),]$Geneid
...
up_VV <- subset(VV_vs_GV_DF, VV_vs_GV_DF$logFC >= 2 & VV_vs_GV_DF$adj.P.Val <= 0.01  & (Geneid %in% VV_vs_GV_DF_Sig))
up_VV <- cbind(up_VV , rep(2, nrow(up_VV)))
colnames(up_VV)[5] <- "Color"

up_GV <- subset(VV_vs_GV_DF, VV_vs_GV_DF$logFC <= -2 & VV_vs_GV_DF$adj.P.Val <= 0.01  & (Geneid %in% VV_vs_GV_DF_Sig))
...
p <- ggplot(data = dfPlot, aes(x = logFC, y = -log10(adj.P.Val), color=Color )) +
  ...
  theme(axis.title.y=element_text(size=20),
        axis.text.y=element_text(size=20))
# Adding lines for significance thresholds
png(filename ="figures/clark_AvgREFs_VV_vs_GV_valcano.png", width= 650, height= 650)
p + geom_hline(yintercept = 2, colour="#000000", linetype="dashed") +
  geom_vline(xintercept = 2, colour="#000000", linetype="dashed") + 
  geom_vline(xintercept = -2, colour="#000000", linetype="dashed")
dev.off()
...
# GG vs VG
GG_vs_VG_DEG <- read.table("./DEGs/DEGs_clark_AvgREFs_fpkm05_GG_vs_VG_fdr1_lfc0.txt", header=TRUE, sep="\t", stringsAsFactors=F) 
GG_vs_VG_DF <- data.frame(GG_vs_VG_DEG$adj.P.Val, GG_vs_VG_DEG$logFC, GG_vs_VG_DEG$Chr, GG_vs_VG_DEG$Geneid)
colnames(GG_vs_VG_DF) <- c("adj.P.Val", "logFC", "Chr", "Geneid")

GG_vs_VG_DF_Sig <- GG_vs_VG_DF[(abs(GG_vs_VG_DF$logFC) >= 2 & GG_vs_VG_DF$adj.P.Val <= 0.01),]$Geneid
...
up_GG <- subset(GG_vs_VG_DF, GG_vs_VG_DF$logFC >= 2 & GG_vs_VG_DF$adj.P.Val <= 0.01  & (Geneid %in% GG_vs_VG_DF_Sig))
up_GG <- cbind(up_GG , rep(2, nrow(up_GG)))
colnames(up_GG)[5] <- "Color"

up_VG <- subset(GG_vs_VG_DF, GG_vs_VG_DF$logFC <= -2 & GG_vs_VG_DF$adj.P.Val <= 0.01  & (Geneid %in% GG_vs_VG_DF_Sig))
up_VG <- cbind(up_VG , rep(3, nrow(up_VG)))
colnames(up_VG)[5] <- "Color"
...
p <- ggplot(data = dfPlot, aes(x = logFC, y = -log10(adj.P.Val), color=Color )) +
  ...
  theme(axis.title.y=element_text(size=20),
        axis.text.y=element_text(size=20))
# Adding lines for significance thresholds
png(filename ="figures/clark_AvgREFs_GG_vs_VG_valcano.png", width= 650, height= 650)
p + geom_hline(yintercept = 2, colour="#000000", linetype="dashed") +
  geom_vline(xintercept = 2, colour="#000000", linetype="dashed") + 
  geom_vline(xintercept = -2, colour="#000000", linetype="dashed")
dev.off()
...
# GG vs GV
GG_vs_GV_DEG <- read.table("./DEGs/DEGs_clark_AvgREFs_fpkm05_GG_vs_GV_fdr1_lfc0.txt", header=TRUE, sep="\t", stringsAsFactors=F) 
GG_vs_GV_DF <- data.frame(GG_vs_GV_DEG$adj.P.Val, GG_vs_GV_DEG$logFC, GG_vs_GV_DEG$Chr, GG_vs_GV_DEG$Geneid)
colnames(GG_vs_GV_DF) <- c("adj.P.Val", "logFC", "Chr", "Geneid")

GG_vs_GV_DF_Sig <- GG_vs_GV_DF[(abs(GG_vs_GV_DF$logFC) >= 2 & GG_vs_GV_DF$adj.P.Val <= 0.01),]$Geneid
...
up_GG <- subset(GG_vs_GV_DF, GG_vs_GV_DF$logFC >= 2 & GG_vs_GV_DF$adj.P.Val <= 0.01  & (Geneid %in% GG_vs_GV_DF_Sig))
up_GG <- cbind(up_GG , rep(2, nrow(up_GG)))
colnames(up_GG)[5] <- "Color"

up_GV <- subset(GG_vs_GV_DF, GG_vs_GV_DF$logFC <= -2 & GG_vs_GV_DF$adj.P.Val <= 0.01  & (Geneid %in% GG_vs_GV_DF_Sig))
up_GV <- cbind(up_GV , rep(3, nrow(up_GV)))
colnames(up_GV)[5] <- "Color"
...
p <- ggplot(data = dfPlot, aes(x = logFC, y = -log10(adj.P.Val), color=Color )) +
  ...
  theme(axis.title.y=element_text(size=20),
        axis.text.y=element_text(size=20))
# Adding lines for significance thresholds
png(filename ="figures/clark_AvgREFs_GG_vs_GV_valcano.png", width= 650, height= 650)
p + geom_hline(yintercept = 2, colour="#000000", linetype="dashed") +
  geom_vline(xintercept = 2, colour="#000000", linetype="dashed") + 
  geom_vline(xintercept = -2, colour="#000000", linetype="dashed")
dev.off()
...
# VG vs GV
VG_vs_GV_DEG <- read.table("./DEGs/DEGs_clark_AvgREFs_fpkm05_VG_vs_GV_fdr1_lfc0.txt", header=TRUE, sep="\t", stringsAsFactors=F) 
VG_vs_GV_DF <- data.frame(VG_vs_GV_DEG$adj.P.Val, VG_vs_GV_DEG$logFC, VG_vs_GV_DEG$Chr, VG_vs_GV_DEG$Geneid)
colnames(VG_vs_GV_DF) <- c("adj.P.Val", "logFC", "Chr", "Geneid")

VG_vs_GV_DF_Sig <- VG_vs_GV_DF[(abs(VG_vs_GV_DF$logFC) >= 2 & VG_vs_GV_DF$adj.P.Val <= 0.01),]$Geneid
...
up_VG <- subset(VG_vs_GV_DF, VG_vs_GV_DF$logFC >= 2 & VG_vs_GV_DF$adj.P.Val <= 0.01  & (Geneid %in% VG_vs_GV_DF_Sig))
up_VG <- cbind(up_VG , rep(2, nrow(up_VG)))
colnames(up_VG)[5] <- "Color"

up_GV <- subset(VG_vs_GV_DF, VG_vs_GV_DF$logFC <= -2 & VG_vs_GV_DF$adj.P.Val <= 0.01  & (Geneid %in% VG_vs_GV_DF_Sig))
up_GV <- cbind(up_GV , rep(3, nrow(up_GV)))
colnames(up_GV)[5] <- "Color"
...
p <- ggplot(data = dfPlot, aes(x = logFC, y = -log10(adj.P.Val), color=Color )) +
  ...
  theme(axis.title.y=element_text(size=20),
        axis.text.y=element_text(size=20))
# Adding lines for significance thresholds
png(filename ="figures/clark_AvgREFs_VG_vs_GV_valcano.png", width= 650, height= 650)
p + geom_hline(yintercept = 2, colour="#000000", linetype="dashed") +
  geom_vline(xintercept = 2, colour="#000000", linetype="dashed") + 
  geom_vline(xintercept = -2, colour="#000000", linetype="dashed")
dev.off()
