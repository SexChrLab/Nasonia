...
GV <- c("SRR2773799", "SRR2773797", "014453", "014454", "014455") # SRR2773798 not included, failed QC
VG <- c("SRR2773794", "SRR2773795", "SRR2773796", "014450", "014451", "014452")
...
GV_Nvit <- paste0("NvitRef/", GV, "_ASEReadCounter_minDepth30_HET_FD_GFF_ratio.bed")
VG_Nvit <- paste0("NvitRef/", VG, "_ASEReadCounter_minDepth30_HET_FD_GFF_ratio.bed")

# For Ngir reference genome, files end with _VgirRef
GV_Ngir <- paste0("NgirRef/", GV, "_ASEReadCounter_minDepth30_HET_FD_GFF_ratio_VgirRef.bed")
VG_Ngir <- paste0("NgirRef/", VG, "_ASEReadCounter_minDepth30_HET_FD_GFF_ratio_VgirRef.bed")
...
GV_sharedSites <- map(GV_lists, ~.$chrPos) %>% #creates a list just of the column of interest
...
GV_sharedSites_ASE = lapply(GV_files, function(x) {
  ASElist <- read.csv(x, header = T, sep = "\t")
  ASElist$chrPos <- paste(ASElist$contig, ASElist$position)
  ASElist <- ASElist[ASElist$chrPos %in% GV_sharedSites,] # only include sites shared among all samples
  return(ASElist)})
...
GV_df$ref_avg <- sapply(colnames, function(x) rowMeans(GV_df [, grep(x, names(GV_df))] )  )

colnames = c("alt_count")
GV_df$alt_avg <- sapply(colnames, function(x) rowMeans(GV_df [, grep(x, names(GV_df))] )  )

colnames = c("total")
GV_df$total_avg <- sapply(colnames, function(x) rowMeans(GV_df [, grep(x, names(GV_df))] )  )
...
GV_merge <-  merge(merge(GV_ref, GV_alt, by.x="geneID", by.y="geneID"), 
                   GV_total, by.x = "geneID",  by.y = "geneID")
...
write.table(GV_merge, file = paste0("avgRefGenomes/", dataGroup, "_GV_ASE_minDepth30.txt"), sep = "\t", 
...
VG_sharedSites <- map(VG_lists, ~.$chrPos) %>% #creates a list just of the column of interest
  reduce(intersect)  #applies the intersect function cumulatively to all the lists shared in ASE
...
VG_sharedSites_ASE = lapply(VG_files, function(x) {
  ASElist <- read.csv(x, header = T, sep = "\t")
  ASElist$chrPos <- paste(ASElist$contig, ASElist$position)
  ASElist <- ASElist[ASElist$chrPos %in% VG_sharedSites,] # only include sites shared among all samples
  return(ASElist)})
...
colnames = c("ref_count")
VG_df$ref_avg <- sapply(colnames, function(x) rowMeans(VG_df [, grep(x, names(VG_df))] )  )

colnames = c("alt_count")
VG_df$alt_avg <- sapply(colnames, function(x) rowMeans(VG_df [, grep(x, names(VG_df))] )  )

colnames = c("total")
VG_df$total_avg <- sapply(colnames, function(x) rowMeans(VG_df [, grep(x, names(VG_df))] )  )
...
VG_merge <-  merge(merge(VG_ref, VG_alt, by.x="geneID", by.y="geneID"), 
                   VG_total, by.x = "geneID",  by.y = "geneID")
...
write.table(VG_merge, file = paste0("avgRefGenomes/", dataGroup, "_VG_ASE_minDepth30.txt"), sep = "\t", 
            row.names = FALSE, quote = FALSE)
...
write.table(sharedSites, file = paste0("avgRefGenomes/", dataGroup, "_GV&VG_sharedSites.txt"), sep = "\t", row.names = FALSE, quote = FALSE)
...
write.table(ASE, file = paste0("avgRefGenomes/", dataGroup, "_ASE_fisherExact.txt"), sep = "\t",
            row.names = FALSE, quote = FALSE)
...
# ASE$sig <- NA
# for (i in 1:nrow(ASE)){
#   x1 <- as.integer(ASE[i, "VG_ref_avg"])# GV
#   x2 <- as.integer(ASE[i, "VG_alt_avg"]) #VG
...
# dim(SIG)
# ASE$FDRp <- p.adjust(ASE$sig, n = length(ASE$sig), method = "fdr")
# FDRp <- subset(ASE, FDRp <= 0.05)
# dim(FDRp)
# write.table(ASE, file = paste0("avgRefGenomes/", dataGroup, "_ASE_VG_binomial.txt"), sep = "\t",
#             row.names = FALSE, quote = FALSE)
# 
# 
# ASE <- GV_merge # VGG
...
#   x2 <- as.integer(ASE[i, "GV_alt_avg"]) #VG
#   test <- binom.test(c(x1, x2), p = 0.5,
#                      alternative = "two.sided",
#                      conf.level = 0.95)
#   test$p.value
...
# FDRp <- subset(ASE, FDRp <= 0.05)
# dim(FDRp)
# write.table(ASE, file = paste0("avgRefGenomes/", dataGroup, "_ASE_GV_binomial.txt"), sep = "\t",
#             row.names = FALSE, quote = FALSE)
#--------
...
#--------
p <- ggplot(ASE) +
  geom_point(data = ASE, aes(x=VG_ratio, y=GV_ratio), alpha = 0.8, size = 4.5, color = "gray70") +
  ...
  labs(
    title = "",
    x = expression(P[VG]~": Nv allellic expression in F"[1]~ "VG"),
    y = expression(P[GV]~": Nv allellic expression in F"[1]~ "GV")
  )+ 
  geom_vline(xintercept = .6, linetype="dotted", 
             color = "lightpink3", size=.9) +
  geom_hline(yintercept=.4, linetype="dotted", 
             color = "lightpink3", size=.9)+
  geom_vline(xintercept = .4, linetype="dotted", 
             color = "cadetblue3", size=.9) +
...
p2 <- p +  annotate("rect", xmin = 0.6, xmax = 1, ymin = 0, ymax = 0.4, fill = "lightpink3", alpha = .5)
p3 <- p2 +  annotate("rect", xmin = 0, xmax = 0.4, ymin = .6, ymax = 1, fill = "cadetblue3", alpha = .5)
p4 <- p3 +  geom_point(data = sigFDR, aes(x=VG_ratio, y=GV_ratio), alpha = 0.75, size = 4.5, color = "darkred", shape = 18 )
p5 <- ggExtra::ggMarginal(p4, type = "histogram", binwidth = .05, fill = GV_Color, xparams = list(fill = VG_Color))
p5

ggsave(paste0("avgRefGenomes/", dataGroup, "_ASE_fisherExact_minDepth30.pdf"), 
       p5,
       width = 5.5, height = 5.5)
dev.off()
dev.off()

# dev.copy(jpeg, filename=paste0("avgRefGenomes/", dataGroup, "_ASE_fisherExact_minDepth30.jpg"),width=8,height=6,units="in", res=1200);
# dev.off()
# dev.off()
#----
...
ggplot(sigFDR, aes(x=Pvg_Pgv)) + geom_histogram(color="black", fill="darkred") +  xlim(c(-1, 1)) 
dev.copy(jpeg, filename=paste0("avgRefGenomes/", dataGroup, "_ASE_fisherExact_histogram_FDR05.jpg"),width=8,height=6,units="in", res=1200);
dev.off()
dev.off()

ggplot(ASE, aes(x=Pvg_Pgv)) + geom_histogram(color="gray") +  xlim(c(-1, 1)) 
dev.copy(jpeg, filename=paste0("avgRefGenomes/", dataGroup, "_ASE_fisherExact_histogram_excludeSIG.jpg"),width=8,height=6,units="in", res=1200);
dev.off()
dev.off()
